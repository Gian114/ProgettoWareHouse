...
class SKUItem{
    ...
    createNewPosition(data){
        return new Promise((resolve, reject) => {
            ...
            this.db.run(sql, [data.id, data.aisle, data.row, data.col, data.maxweight, data.maxvolume], (err) => {
                if (err) {
                  reject(err);
                  return;
                }
    ...
    modifyPosition(id, data){
        return new Promise((resolve, reject)=>{
        const sql = 'UPDATE POSITION SET aisle = ?, row = ?, col = ?, max_weight = ?, max_volume = ?, occupied_weight = ?, occupied_volume = ? WHERE ID = ?'
        this.db.run(sql, [data.aisle, data.row, data.col, data.max_weight, data.max_volume, data.occupied_weight, data.occupied_volume,id], (err, r)=>{
            if (err) {
                reject(err);
                return;
            }
