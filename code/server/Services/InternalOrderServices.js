...
class InternalOrderServices {
    async getInternalOrders() {
        let internal_orders;
        internal_orders = this.transformInternalOrdersListNotCompleted(await io_table.getInternalOrdersNotCompleted());
        const io_comp = this.transformInternalOrdersListCompleted(await io_table.getInternalOrdersCompleted());
    ...
    async getInternalOrdersByState(state) {
        ...
        const internal_orders = this.transformInternalOrdersListNotCompleted(await io_table.getInternalOrdersByState(state));
        return internal_orders;
    }
    ...
    async getInternalOrderById(id) {
        ...
        if (state === '') {
            ...
            internal_order = this.transformInternalOrderCompleted(await io_table.getInternalOrderCompletedById(id));
        }
        else {
            internal_order = this.transformInternalOrderNotCompleted(await io_table.getInternalOrderNotCompletedById(id));
        }
    ...
    async createInternalOrder(issue_date, state, customer_id, products) {
        const internal_order_id = await io_table.createInternalOrder(issue_date, state, customer_id);
    ...
    async modifyInternalOrder(id, new_state, sku_items = undefined) {
        ...
        await io_table.modifyInternalOrderState(id, new_state);
